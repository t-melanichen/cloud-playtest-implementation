...
- **7-day max duration cap for streaming playtests** — enforce the project rule that streaming-enabled playtests cannot run longer than 7 days and cannot use `noEndDate`. The current branch adds a separate cloud-streaming end-date cap; remaining planned work should align this with the overall Playtest start/end-date validation (`playtestFormSchema.ts` / `PlaytestDatesField.tsx`) per spec.
