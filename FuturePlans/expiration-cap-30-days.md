...
- [ ] **UX (warn on over-cap end date):** Surface a clear message when the creator picks an end date past the max streaming window, so a playtest doesn't *look* like it runs for two weeks while the streaming link only works for one. Raised by Anthony Keller on PR 15834601 (thread 128136674): "we may want to warn the user they can't choose an end date past the max duration ... so we don't have a Playtest that looks like it runs for 2 weeks, but the streaming link only works for a week." Backend currently just clamps `ExpirationTime` server-side in `XCloudPlaytestTitleIngestionBuilder`; the friendly warning is a front-end follow-up.
