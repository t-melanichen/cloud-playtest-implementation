...
- [ ] **B1 — PC-playtest SUG.** The poll filter's SUG is left unconstrained (in-code TODO 62521491). Once a `PC_playtest` SUG constant is exposed in `Services.Common.Ids`, set `GameStreamingServerFilter.SystemUpdateGroups = [pcPlaytestSug]`. *(Timi: "we will have a distinct PC underscore play test SUG.")* — **PR**.
- [ ] **B2 — Env-specific region.** Replace the interim `IsProd() ? WestUs2 : WestEurope` with real region config (set `GameStreamingServerFilter.Regions` on the PC branch): test/int = WestUs2 + WestEurope; prod = NorthCentralUs. (Shared TODO; affects Xbox + PC branches.) See [`FuturePlans/region-configuration-routing.md`](./region-configuration-routing.md).
