...
| **PT2** | Construct `StoreAsset` from playtest + package data | Mostly already in `PlaytestLifecycleState` + `PlaytestResponse`. New: `XboxTitleId` resolver (P0 blocker), `AumID` default-or-resolver (v1 acceptable with constant; full plumbing v1.1). | 4 |
