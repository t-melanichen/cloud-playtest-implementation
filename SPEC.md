...
| 3 | **xCloud Ingestion Workflow** | A new `PlaytestTitleIngestionWorkflow` runs inside `services.contentingestion`. The stages are **strictly sequential** — partner-registry writes only fire after asset ingestion is fully ready: (a) validates inbound parameters; (b) chains the existing `AssetIngestionWorkflow` using the supplied `StoreAsset` (no BigCat lookup) — this stage creates the asset, ingests every version into SUCU, and updates the flighted-content mappings; (c) **waits for asset ingestion to reach a terminal "ready" state** (`PollVersionIngestionJobsAsync` in `AssetIngestionWorkflow.cs:162-186` must complete) before touching Partner Registry; (d) **only then** upserts the playtest-specific offering `xpt-{shortId}` with `AllowedDnaGroups` + `AllowedSandboxId` + `ExpirationTime`, and (e) attaches the title to that offering. The offering upsert and title attach are two distinct writes but are combined into a single ADO PR via `BulkEditAsync` to halve the SFI manual-approval cost (one PR, two changes — the offering still has to exist for the attach to be meaningful). Finally (f) polls the install pipeline for capacity. | xCloud |
...
| `ContentId` | `PlaytestLifecycleState.PlaytestContentId` | In the XProduct doc this corresponds to `packages.{packageId\|versionId}.contentId`. For a playtest product the published XProduct typically contains multiple package entries, some with `null` `contentId` (verified against `Files/XProductDocument.json`). xPlaytest selects the package using these rules: (1) filter `packages` to entries whose `platformDependencies[].platformName` includes the publish platform (v1 = `WINDOWS.DESKTOP`); (2) keep only entries with a non-null `contentId`; (3) if multiple remain, choose deterministically by the lexicographically-smallest `{packageId\|versionId}` map key. The XProduct `packages.{packageId\|versionId}` object exposes no package `state` or upload-timestamp field, so selection is platform + content-derived only. If `contentId` is null on the chosen entry, fall back to `servicingContentId` (also on the `packages.{packageId\|versionId}` object) — both fields are produced by the same downstream BigCat pipeline and the playtest workflow accepts either. The selected value must equal `SourceId` derived inside the workflow. |
| `XboxTitleId` | XProduct `alternateIds[idType=XboxTitleId].value` (verified path in `Files/XProductDocument.json`; string-typed in the doc, cast to `uint`) | **P0 blocker** — validator throws if null/0 for games. `PlaytestProductDocumentBuilder.cs:53-70` currently hardcodes empty. |
